...
export default function PaymentsPage() {
  ...
  const staffWithBalance = staff.filter((s) => s.unpaidBalance > 0 || s.walletBalance > 0);
    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">{payments.length} transactions recorded</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="stat-card">
          ...
          <div><p className="text-xs text-gray-500">Total Unpaid</p><p className="text-lg sm:text-xl font-bold text-red-700">Rs. {totalUnpaid.toFixed(0)}</p></div>
        </div>
      </div>

      {/* Staff Balances Quick View */}
      {staffWithBalance.length > 0 && (
        <div className="card">
          <h3 className="font-semibold text-gray-900 mb-3 text-sm">Staff Balance Overview</h3>

          {/* Mobile: stacked cards */}
          <div className="md:hidden space-y-2">
            {staffWithBalance.map((s) => (
              <div key={s._id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                ...
                <div className="flex items-center gap-3 text-sm">
                  {s.walletBalance > 0 && (
                    <span className="text-green-700 font-semibold">+{s.walletBalance.toFixed(0)}</span>
                  )}
                  {s.unpaidBalance > 0 && (
                    <span className="text-red-600 font-semibold">-{s.unpaidBalance.toFixed(0)}</span>
                  )}
                </div>
              </div>
            ))}
              ...
              <tbody className="divide-y divide-gray-50">
                {staffWithBalance.map((s) => (
                  <tr key={s._id} className="hover:bg-gray-50">
                    <td className="table-cell font-medium">{s.fullName}</td>
                    <td className="table-cell text-gray-500 text-xs">{s.department}</td>
                    <td className="table-cell">
                      {s.walletBalance > 0 ? (
                        <span className="text-green-700 font-semibold">Rs. {s.walletBalance.toFixed(2)}</span>
                      ) : <span className="text-gray-300">—</span>}
                    </td>
                    <td className="table-cell">
                      {s.unpaidBalance > 0 ? (
                        <span className="text-red-600 font-semibold">Rs. {s.unpaidBalance.toFixed(2)}</span>
                      ) : <span className="text-gray-300">—</span>}
                    </td>
                  </tr>
                ))}
      ...
      <div className="card p-0 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input className="input pl-9 text-sm" placeholder="Search by staff name..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <select className="input text-sm w-full sm:w-48" value={filterType} onChange={(e) => setFilterType(e.target.value)}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div><label className="label">Amount (Rs.) *</label><input type="number" className="input" placeholder="0.00" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} /></div>
              <div><label className="label">Note (Optional)</label><input className="input" placeholder="Payment reference..." value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} /></div>
              ...
              {form.paymentType === 'CreditSettlement' && form.amount && (
                <div className="bg-orange-50 rounded-lg p-3 text-sm text-orange-700">
                  Rs. {parseFloat(form.amount || '0').toFixed(2)} will be deducted from unpaid balance.
                </div>
              )}
            </div>
